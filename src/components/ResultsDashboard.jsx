...
import { getCountryInfo, getCountryFactors } from '../services/countries';
...
function getPercentile(totalKg, avgWeekly = 182) {
  const z = (avgWeekly - totalKg) / 60;
...
function emotionColor(total, avgWeekly = 182) {
  if (total <= PARIS_TARGET) return { ring: '#22c55e', glow: 'rgba(34,197,94,0.08)', label: 'Excellent' };
  if (total <= avgWeekly * 0.7) return { ring: '#d4a017', glow: 'rgba(212,160,23,0.06)', label: 'Good' };
  if (total <= avgWeekly) return { ring: '#f59e0b', glow: 'rgba(245,158,11,0.06)', label: 'Average' };
...
export default function ResultsDashboard({ carbonData, recommendations, inputs, onBack, onRecalculate }) {
  ...
  const countryCode = inputs?.country || 'us';
  const countryInfo = getCountryInfo(countryCode);
  const countryBenchmarks = getCountryFactors(countryCode).benchmarks || { transport: 77, energy: 51, flight: 10, diet: 39.4, lifestyle: 13 };
  const US_AVG_WEEKLY = countryBenchmarks.transport + countryBenchmarks.energy + countryBenchmarks.flight + countryBenchmarks.diet + countryBenchmarks.lifestyle;
  const emotion = emotionColor(carbonData.totalKg, US_AVG_WEEKLY);
  const revealed = (stage) => revealStage >= REVEAL_STAGES.indexOf(stage);
  const scoreTier = getScoreTier(recs?.score || 0);
  const percentile = getPercentile(carbonData.totalKg, US_AVG_WEEKLY);
      ...
      <div className="section-shell">
        <div className="relative z-10 max-w-5xl mx-auto">
          ...
          >
            <motion.button
              onClick={onBack}
              className="flex items-center gap-2 text-xs text-white/40 hover:text-white/70 transition-colors group uppercase tracking-widest focus-ring"
            >
            ...
            <div className="flex items-center gap-3">
              ...
              <motion.button
                ...
                className="px-4 py-1.5 rounded-lg bg-white/[0.04] border border-white/[0.06] text-xs text-white/50 hover:text-white/80 hover:border-white/[0.12] transition-all uppercase tracking-wider font-medium btn-gold-line btn-premium focus-ring"
              >
                Recalculate
              </motion.button>
            </div>
          ...
          <motion.div
            ...
            className="mb-8"
          >
            <div className="rounded-2xl overflow-hidden panel-surface card-corner-draw card-gold-glow">
          ...
          <AnimatePresence>
            {revealed('comparison') && (
              <motion.div
                ...
                className="rounded-2xl p-6 md:p-8 mb-8 space-y-4 panel-surface"
              >
                ...
                <ComparisonBar label={`${countryInfo.flag} ${countryInfo.name} Average`} value={US_AVG_WEEKLY} maxValue={barMax} color="rgba(255,255,255,0.15)" delay={0.08} icon={countryInfo.flag} />
          ...
          <AnimatePresence>
            {revealed('breakdown') && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.5, ease: [0.22, 1, 0.36, 1] }}
                className="rounded-2xl p-6 md:p-8 mb-8 panel-surface"
              >
          ...
          <AnimatePresence>
            {revealed('full') && (
              ...
              >
                {/* Tabs */}
                <div className="flex gap-1 mb-8 p-1 rounded-xl panel-surface-soft overflow-x-auto">
                  {tabs.map((tab) => (
                    <button
                      key={tab.id}
                ...
                <AnimatePresence mode="wait">
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                        {[
                          ...
                          <motion.div
                            ...
                            className="rounded-xl p-5 text-center panel-surface"
                          >
                      ...
                      <div className="grid lg:grid-cols-5 gap-4">
                        <motion.div
                          ...
                          className="lg:col-span-3 rounded-2xl p-6 md:p-8 panel-surface"
                        >
                        ...
                        <motion.div
                          ...
                          className="lg:col-span-2 rounded-2xl p-6 md:p-8 flex flex-col panel-surface"
                        >
                  ...
                  {activeTab === 'breakdown' && (
                    ...
                    >
                      <motion.div
                        ...
                        className="rounded-2xl p-6 md:p-8 panel-surface"
                      >
                          ...
                          <div className={`grid sm:grid-cols-2 ${colClass} gap-3`}>
                            {breakdownCards.map((c, i) => (
                              <motion.div
                                ...
                                className="rounded-xl p-4 panel-surface"
                              >
                  ...
                  {activeTab === 'recommendations' && (
                    ...
                    >
                      {aiLoading ? (
                        <div className="rounded-2xl p-12 text-center relative overflow-hidden panel-surface">
                        ...
                        <>
                          ...
                          {recs.recommendations.map((rec, i) => (
                            <motion.div
                              ...
                              animate={{ opacity: 1, y: 0 }}
                              transition={{ delay: i * 0.06, duration: 0.4, ease: [0.22, 1, 0.36, 1] }}
                              className="rounded-xl p-5 flex gap-4 items-start hover:border-white/[0.10] transition-colors panel-surface"
                            >
                        ...
                        <div className="rounded-2xl p-12 text-center panel-surface">
